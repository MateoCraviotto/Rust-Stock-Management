...
use tp::{ecommerce::args::Args, log_level, common::order::Order, ecommerce::{purchases::purchase_state::PurchaseState, network::connection::{write_socket, read_socket}}};
...
async fn main() -> anyhow::Result<()>{
    ...
    println!("Running E-Commerce which will connect to {}:{}", args.ip, store_port);
    let client = Arc::new(Mutex::new(TcpStream::connect(format!("{}:{}", args.ip, store_port)).await?));
...
async fn manage_purchase(order: Order, store: Arc<Mutex<TcpStream>>) -> anyhow::Result<PurchaseState> {
    ...
    let answer = match read_socket(store.clone()).await {
        Ok(answer) => {
            println!("Store answered {} to order {:?}", answer, order.get_product());
            answer
        },
        Err(e) => {
    ...
    match answer_state {
        PurchaseState::Reserve => {
            ...
            let answer = match read_socket(store.clone()).await {
                Ok(answer) => {
                    println!("Store answered: {} to order {:?}", answer, order.to_string());
                    answer
                },
                Err(e) => {
                    println!("Error while reading answer from store: {}", e);
            ...
            match confirmation_state {
                ...
                _ => {
                    println!("Purchase was not confirmed. Cancelling purchase {:?}.", order.to_string()); // Should look in next store
